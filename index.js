...
class RubyGen extends yeoman_generator_1.default {
    ...
    async prompting() {
        ...
        const answer = await this.prompt([
            ...
            },
            {
                ...
                default: C.kebab(path_1.default.basename(this.destinationRoot())),
            },
            {
                type: 'input',
                message: 'description?',
            ...
            },
            {
                ...
                default: 'rupertofly',
                validate: (input) => {
                    const valid = input.length < 64 && /^[^A-Z,@]+$/.test(input);
        ...
        console.log(path_1.default.basename(this.destinationRoot()));
        console.log(answer);
        this.answers = answer;
    }
    async writing() {
    ...
    async install() {
        ...
        if (this.answers.d3)
            deps.push('d3');
        if (this.answers.webpack)
            devDeps.push('webpack', 'webpack-merge', 'ts-loader', 'css-loader', 'sass-loader', 'style-loader', 'html-webpack-plugin', 'mini-css-extract-plugin', 'pug', 'pug-loader', 'tsconfig-paths-webpack-plugin');
        if (this.answers.sb) {
